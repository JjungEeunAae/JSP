...
public class mamber extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		for(int i = 0 ; i < list.size(); i++) {
			json += "{\"id\":\"" + list.get(i).getMemberId()+"\"" + ",\"pw\":\"" + list.get(i).getMemberPw() + "\""
					+ ",\"name\":\"" + list.get(i).getMemberName() + "\"" + ",\"addr\":\"" + list.get(i).getMemberAddr()
					+ "\"" + ",\"tel\":\"" + list.get(i).getMemberTel() + "\"" + ",\"birth\":\""
					+ list.get(i).getMemberBirth() + "\"" + ",\"mail\":\""
					+ list.get(i).getMemberEmail() + "\"" +"}";
			if(i + 1 != list.size()) {
				json += ",";
			}
		}
